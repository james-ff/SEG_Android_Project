...
/**
 * This class works as a data storage for the indicators that will be presented
 * to user on selecting a country. This should be properly cached and stored so
 * to avoid increased traffic usage.
 * 
...
public class ListOfIndicators {
	...
	public static final String CATEGORY_BUSINESS = "Business",
			CATEGORY_CITY_LIFE = "City Life", CATEGORY_CLIMATE = "Climate",
	...
	/**
	 * Static Map containing the indicator categories, subcategories, indicator
	 * readable names and indicator codes.
	 */
	...
	public ListOfIndicators() {
		// Category Setup -- These categories will be the headings of the
		// expandable groups (use droneStrikes.keySet())
		...
		/*
		 * -- Indicators Setup -- Here we add the known codes of indicators we
		 * are interested in. --
		 */
		...
		reference.put("SH.STA.ACSN.UR", "% of urban population with access to improved Sanitation Facilities");
	...
	private static boolean addIndicator(String category, String code) {
		try {
			...
			Log.e("IndicatorGrab", "An error occured adding indicator with code: "
					+ code);
			e.printStackTrace();
			return false;
		}
	...
	public static boolean loadIndicatorsForCategory(final String category) {
		...
		if (subCategory == null || subCategory.size() == 0) {
			Log.w("IndicatorLoad", "No indicators are present for category name: "
					+ category);
			return false;
		} else {
	...
	public static List<Indicator> getAllLoadedIndicatorsFromCategory(String category) {
		...
		if (indicators.size() == 0) {
			Log.w("IndicatorGrab", "Found no indicators with category: "
					+ category);
		}
		return indicators;
	}
	...
	public static String[] getCategoriesAsArray() {
		return new String[] { CATEGORY_BUSINESS, CATEGORY_CITY_LIFE,
				CATEGORY_CLIMATE, CATEGORY_DEMOGRAPHICS, CATEGORY_EDUCATION,
				CATEGORY_EMPLOYMENT_PROSPECTS, CATEGORY_FINANCE,
				CATEGORY_QUALITY_OF_LIFE, CATEGORY_RURAL_LIFE };
	}
